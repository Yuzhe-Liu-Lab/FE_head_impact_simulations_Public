...
%% 加载 Info.mat / Load Info.mat
p = load('info.mat');   
Info = p.Info;    
num_Case = length(Info); % 获取实验案例的总数 / Get the total number of cases

%% 设定项目目录 / Define Project Directories
Dir_Project = Info(1).Dir_Project;  % 项目主目录 / Project main directory

Dir_Scripts = strcat(Dir_Project, '\Scripts');  % 脚本目录 / Scripts directory
Dir_Data = strcat(Dir_Project, '\Data');        % 数据目录 / Data directory
Dir_Data_Kinematics = strcat(Dir_Project, '\Data\Kinematics'); % 运动学数据目录 / Kinematics data directory

% 如果 Kinematics 目录不存在，则创建 / Create Kinematics directory if it does not exist
if ~exist(Dir_Data_Kinematics, 'dir')  
    mkdir(Dir_Data_Kinematics);  
end  

%% 加载原始运动学数据，并组装 mg_data / Load original kinematic data and assemble mg_data
mg_data1 = struct([]);

for i_Case = 1:num_Case
    mg_data1(i_Case).t = Info(i_Case).t;  % 时间序列 / Time sequence
    num_t = length(mg_data1(i_Case).t);  % 计算时间点数目 / Number of time points

    % 初始化角速度、角加速度、线性加速度矩阵 / Initialize angular velocity, angular acceleration, and linear acceleration matrices
    mg_data1(i_Case).ang_vel = zeros(num_t, 3);  
    mg_data1(i_Case).ang_acc = zeros(num_t, 3);  
    mg_data1(i_Case).lin_acc_CG = zeros(num_t, 3);  

    % 读取当前案例的加速度和角速度信息 / Load acceleration and angular velocity information for the current case
    ...
    % 载荷时间 / Load duration
    T_i = Info(i_Case).T;  
    Omega_i = 2 * pi / T_i;  % 计算角频率 / Compute angular frequency
    
    % 找到合适的时间索引 / Find appropriate time index
    ind_T_i = find(T_i > mg_data1(i_Case).t);
    ind_T_i = ind_T_i(end);  
    t_1 = mg_data1(i_Case).t(1:ind_T_i);  

    % 计算角速度、角加速度、线性加速度 / Compute angular velocity, angular acceleration, and linear acceleration
    ...
    % 填充数据矩阵 / Populate data matrices
    mg_data1(i_Case).ang_vel = [ang_vel_0; zeros(num_t - ind_T_i, 3)];
    mg_data1(i_Case).ang_acc = [ang_acc_0; zeros(num_t - ind_T_i, 3)];
    mg_data1(i_Case).lin_acc_CG = [lin_acc_CG_0; zeros(num_t - ind_T_i, 3)];
end


%% 调整时间轴 / Shift the time domain
...
for i_Case = 1:num_Case
    ...
    t_shift = -t0(1);  % 让时间起点归零 / Shift the starting time to zero
    t0 = t0 + t_shift;  
    mg_data2(i_Case).t = (Info(i_Case).t)';  

    % 对头部质心的线性加速度、角速度和角加速度进行插值
    % / Interpolate linear acceleration at the head center, angular velocity, and angular acceleration
...
%% 计算大脑质心处的线性加速度 / Transfer linear acceleration from head center to brain center
for i_Case = 1:num_Case
    % 读取头部和大脑质心坐标 / Load head and brain center coordinates
    Head_Center = Info(i_Case).Head_Center;
    Brain_Center = Info(i_Case).Brain_Center;

    % 读取头部质心处的线性加速度、角速度和角加速度 / Load linear acceleration, angular velocity, and angular acceleration at head center
    lin_acc_HCG = mg_data2(i_Case).lin_acc_CG;  
    ang_vel = mg_data2(i_Case).ang_vel;  
    ang_acc = mg_data2(i_Case).ang_acc;  

    % 计算从头部质心到大脑质心的线性加速度转换 / Compute linear acceleration transformation from head center to brain center
    lin_acc_BCG = Transfer_Location_lin_acc(lin_acc_HCG, ang_vel, ang_acc, Head_Center, Brain_Center);

    % 赋值到 mg_data 结构体 / Assign to mg_data structure
    mg_data2(i_Case).lin_acc_BCG = lin_acc_BCG;
end

%% 转换至全局坐标系 / Convert to global coordinate system
mg_data3 = Transfer_Reference_Local2Global_mg_data(mg_data2);  
% 以 "G_" 开头的参数表示在全局坐标系下 / Parameters starting with "G_" are in the global coordinate system
...
%%%%% 机器学习数据准备 / Prepare Data for Machine Learning %%%%%
impact = struct([]);

for i_Case = 1:length(mg_data)
    % 仅保留机器学习所需的关键变量 / Retain only key variables needed for machine learning
...
% 保存用于机器学习的数据 / Save data for machine learning
